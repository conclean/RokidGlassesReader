...
在 CUSTOMAPP 会话下，指定眼镜端目标包名，通过 SDK 在眼镜上**查询安装状态、上传安装 APK、启动 Activity、停止、卸载**。目标应用在眼镜端成功启动并处于可交互态后，即完成 CustomApp 会话的会话构建，此后可调用音频、拍照与自定义指令。

## 前置条件

- 已完成[连接与会话](/07-连接与会话.md)配置，`configCXRSession` 使用 CUSTOMAPP 与正确的 packageName。
...
> 会话构建完成通常以 `onOpenAppResult(true)` 或 `onGlassAppResume(true)` 为准，此后可进入音频、拍照、自定义指令。
...
- `context.getExternalFilesDir("DCIM/Rokid")/cxrL.apk` — **推荐**，通常无需额外存储权限
- `context.filesDir/cxrL.apk`

若使用公共目录（如 `/sdcard/DCIM/Rokid/cxrL.apk`），须先完成 [SDK 导入](/05-SDK导入.md)专章中的存储权限申请，并确认文件 `canRead()` 后再调用 `appUploadAndInstall`。RenewCXRLSample 通过 `ApkInstallAccess` 过滤不可读路径，并在 CxrSessionActivity 中引导「所有文件访问」或 `READ_EXTERNAL_STORAGE`。
...
SDK 导入见[眼镜端（CXR-S）](/05-SDK导入.md)章节；自定义指令见[自定义指令](/12-自定义指令.md)专章。
...
4. 此后可进入音频、拍照、自定义指令。
5. 眼镜端 MainActivity.onCreate 中创建 MainViewModel，其 init 块注册 CXRServiceBridge 订阅，即可接收手机指令。
...
- 自定义指令须复用同一会话的 CXRLink，见[自定义指令](/12-自定义指令.md)专章。
- 须在应用启动成功（会话构建完成）后再调用音频、拍照、自定义指令。

## 下一步

- [音频](/10-音频.md) / [拍照](/11-拍照.md) — CustomApp 会话构建完成后可用
- [自定义指令](/12-自定义指令.md) — 仅 CustomApp 会话
- [眼镜端按键与系统广播](/13-眼镜端按键与系统广播.md) — 按键上报
