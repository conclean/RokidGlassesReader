...
在已建立的 CXRLink 会话上注册图片流回调，调用 `takePhoto(width, height, quality)` 触发眼镜拍照，通过 `IImageStreamCbk.onImageReceived` 接收 JPEG 压缩字节并在手机端解码展示或保存。

## 前置条件

- 全局 CXRLink 已创建且 `connect` 完成。
- **会话构建已完成**（与[音频](/11-音频.md)专章相同）：
